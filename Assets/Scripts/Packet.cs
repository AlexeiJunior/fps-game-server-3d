...
public class Packet {
    ...
    public void Write(Keys keys) {
        foreach(var prop in keys.GetType().GetProperties()) {
            ...
            if(type == typeof(int)) Write((int) value);
    ...
    public Keys ReadKeys(bool moveReadPos = true) {
        ...
        foreach(var prop in keys.GetType().GetProperties()) {
            ...
            if(type == typeof(int)) prop.SetValue(keys, ReadInt(moveReadPos));
            if(type == typeof(Vector3))prop.SetValue(keys, ReadVector3(moveReadPos));
            if(type == typeof(Quaternion))prop.SetValue(keys, ReadQuaternion(moveReadPos));
        }
